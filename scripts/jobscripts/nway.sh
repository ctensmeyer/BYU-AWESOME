...
prediction_script=/fslhome/waldol1/fsl_groups/fslg_icdar/compute/BYU-AWESOME/scripts/prediction/nway_prediction.py
eval_script=/fslhome/waldol1/fsl_groups/fslg_icdar/compute/BYU-AWESOME/scripts/prediction/nway_eval.py
...
do
	...
	# run nway evaluation over each set of output
	for d in $out_dir/$split/nway/*
	do
		gt=`basename $d | rev | cut -d_ -f3- | rev`
		base=`basename $d`
		python $eval_script $d $dataset_dir/$gt $out_dir/$split/results/${base}.txt $out_dir/$split/results/${base}_summary.txt $num_classes
