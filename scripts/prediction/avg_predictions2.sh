...
do
	...
	echo $binary_dir1
	for d in $binary_dir1/processed_gt*
	do
		...
		for im in $in_dir/1/predictions/$split/binary/$gt/*
		do
			base=`basename $im`
			echo $base
			python avg_ims.py $out_dir/$base $in_dir/*/predictions/$split/binary/$gt/$base
			python invert.py $out_dir/$base $out_dir/$base
		done
		python $metric_script $out_dir $dataset_dir/pr_dats $results_dir/${gt}.txt $results_dir/${gt}_summary.txt
	done
done
